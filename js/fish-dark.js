/**
 * Fish animation for Hexo Butterfly theme.
 * Adapted from xiabo2/CDN to support dark/light theme auto-switching.
 *
 * Usage: include this single script in inject.bottom (no need for separate
 *        fishes.js loader or jQuery dependency).
 *
 * Colour behaviour:
 *   - Light mode (default):  fish & wave fill  hsl(0, 0%, 95%)   — near-white
 *   - Dark  mode:            fish & wave fill  hsl(0, 0%, 18%)   — dark grey
 *
 * The script watches for Butterfly's data-theme attribute changes and
 * smoothly updates the canvas colour in real-time.
 */
...
(function () {
  ...
  function bootstrap() {
    ...
    var wrap = document.getElementById('footer-wrap');
    if (wrap) {
  ...
  var RENDERER = {
    ...
    init: function () {
      ...
      // Watch for Butterfly theme changes
      this.observeTheme();
    },
    ...
    createSurfacePoints: function () {
      ...
      for (var i = 0; i < count; i++) {
        var point = new SURFACE_POINT(this, this.pointInterval * i);
        if (i > 0) {
          point.setPreviousPoint(this.points[i - 1]);
        }
        if (i < count - 1 && this.points[i + 1]) {
          point.setNextPoint(this.points[i + 1]);
        }
        this.points.push(point);
      }
      // Link neighbours
      for (var j = 0; j < this.points.length; j++) {
        if (j > 0) this.points[j].setPreviousPoint(this.points[j - 1]);
        if (j < this.points.length - 1) this.points[j].setNextPoint(this.points[j + 1]);
      }
    },
    ...
    watchWindowSize: function () {
      ...
      this.watchIds.push(setTimeout(this.jdugeToStopResize, 500));
    },
    ...
    render: function () {
      ...
      this.context.fillStyle = this.fillColor || getFillColor();
  ...
  SURFACE_POINT.prototype = {
    ...
    interfere: function (y, velocity) {
      this.fy = this.renderer.height * this.ACCELARATION_RATE *
        ((this.renderer.height - this.height - y) >= 0 ? -1 : 1) * Math.abs(velocity);
    },
